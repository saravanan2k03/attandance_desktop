...
import 'package:act/Features/Report/attendance_generated.dart';
...
class _AttendancePdfScreenState extends State<AttendancePdfScreen>
  ...
  Widget _buildSummaryCards() {
    ...
    int overtimeCount =
        widget.attendanceData.records.where((r) => r.isOvertime).length;
        ...
        child: Column(
          ...
          children: [
            ...
            ...departmentCounts.entries.map((entry) {
              double percentage =
                  (entry.value / widget.attendanceData.records.length) * 100;
              return Padding(
                  ...
                  children: [
                    ...
                    LinearProgressIndicator(
                      ...
                      valueColor: AlwaysStoppedAnimation<Color>(
                        Colors.primaries[departmentCounts.keys.toList().indexOf(
                              entry.key,
                            ) %
  ...
  Widget _buildRecentRecords() {
    List<AttendanceRecord> recentRecords =
        widget.attendanceData.records.take(5).toList();
  ...
  Widget _buildEmployeeCard(
    ...
    int presentDays =
        records.where((r) => r.presentOne == '1' || r.presentTwo == '1').length;
    double attendanceRate = (presentDays / records.length) * 100;
    double totalHours = records.fold(
      0,
      (sum, r) => sum + (double.tryParse(r.workHours) ?? 0),
    );
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 20),
          Expanded(child: _buildMonthlyPreview()),
        ],
      ),
    );
  }
            ...
            Expanded(
              child: ListView(
                children: [
                  _buildPreviewItem(
                    'Total Employees',
                    '${widget.attendanceData.records.map((r) => r.employeeId).toSet().length}',
                  ),
                  _buildPreviewItem(
                    'Total Records',
                    '${widget.attendanceData.records.length}',
                  ),
                  _buildPreviewItem(
                    'Working Days',
                    '22',
                  ...
                  _buildPreviewItem(
                    'Average Attendance',
                    '${((widget.attendanceData.records.where((r) => r.presentOne == '1' || r.presentTwo == '1').length / widget.attendanceData.records.length) * 100).toStringAsFixed(1)}%',
                  ),
                ],
              ),
            ),
          ...
          (context) => AlertDialog(
            ...
            content: Column(
              ...
              children: [
                ...
                Text(
                  'Present Days: ${records.where((r) => r.presentOne == '1' || r.presentTwo == '1').length}',
                ),
                Text(
                  'Overtime Days: ${records.where((r) => r.isOvertime).length}',
                ),
              ],
            ),
            actions: [
