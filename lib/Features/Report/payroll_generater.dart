...
class PayrollPdfGenerator {
        ...
        pw.Row(
          ...
          children: [
            pw.Column(
              ...
              children: [
                pw.Text(
                  'Organization: ${payrollData.organization}',
                  style: pw.TextStyle(
                    fontSize: 14,
                    fontWeight: pw.FontWeight.bold,
                  ),
  ...
  static pw.Widget _buildSummarySection(PayrollListModel payrollData) {
    double totalBasicSalary = payrollData.payrollRecords.fold(
      0,
      (sum, record) => sum + record.basicSalary,
    );
    double totalNetSalary = payrollData.payrollRecords.fold(
      0,
      (sum, record) => sum + record.netSalary,
    );
    double totalOvertimeSalary = payrollData.payrollRecords.fold(
      0,
      (sum, record) => sum + record.overtimeSalary,
    );
    int generatedCount =
        payrollData.payrollRecords
            .where((record) => record.payrollGenerated)
            .length;
        ...
        children: [
          ...
          pw.Row(
            ...
            children: [
              ...
              _buildSummaryItem(
                'Generated',
                '$generatedCount/${payrollData.payrollRecords.length}',
              ),
            ],
          ),
        ],
  ...
  Future<void> generateEmployeePayslip(
    ...
    await _savePdf(
      pdf,
      'payslip_${employee.employeeName}_${employee.month}.pdf',
    );
  }
